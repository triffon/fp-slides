...
\begin{frame}
  ...
  \begin{definition}
    ...
    \item \lst{fmap f . fmap g} \eqv {fmap (f . g)} (дистрибутивност относно композиция)
    \end{enumerate}
  \end{definition}
  \pause
...
class (Functor f) => Applicative f where
  ...
  (<*>) :: f (a -> b) -> f a -> f b
\end{lstlisting}
  % TODO съкращаване на a и f
  \pause
  Можем да дефинираме \lst{fmap f a = pure f <*> a}.\\
  \pause
...
\begin{lstlisting}
class (Applicaive m) => Monad m where  
  return :: a -> m a  
  return = pure     
...
\begin{frame}
  ...
  \end{itemize}
  \onslide<+->
  \lst{do} синтаксисът работи за всички екземпляри на \lst{Monad}!
\end{frame}
...
\begin{frame}[fragile]
  ...
  \item \lst{join :: (Monad m) => m (m a) -> m a}
    ...
    \item ``слива'' двойната опаковка в единична
      % TODO: съкращаване на mm
    \item \lst{join mm = mm >>= id}
  ...
  \item \lst{foldM :: (Monad m) => (a -> b -> m a) -> a -> [b] -> m a}
    ...
    \item
\begin{lstlisting}
boundSum lim = foldM (\x y -> if x+y < lim then Just (x+y)
                              else Nothing) 0
\end{lstlisting}
