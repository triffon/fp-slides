...
\begin{frame}
  ...
  \renewcommand{\lra}{\onslide<+->\longrightarrow}
  \begin{itemize}[<+->]
...
\tt{\underline{g [3]}}
\pause
&\lra \underline{\tt{f (head [3]) (head (tail [3]))}}\\
\pause
...
\begin{frame}
  ...
  \item първо се оценява $f$, за да разберем как да продължим
...
\begin{frame}
  ...
  \item \evalstops{(f 0)}{\textsf{забива, но не изразходва памет}}
  \item \tt f е \textbf{опашково-рекурсивна} и се реализира чрез итерация
  \item \evalstos{(f 0)}{\evalstos{(f 1)}{\evalstos{(f 0)}{\evalstos{(f 1)}{...}}}}
  \end{itemize}
  \onslide<+->
  В Haskell:
  \begin{itemize}[<+->]
  \item \tt{f x = f (1-x)}
  \item \evalstops{f 0}{\textsf{\alert{забива с изтичане на памет!}}}
  \item \evalstos{f 0}{\evalstos{f (1-0)}{\evalstos{f (1-(1-0))}{\evalstos{f (1-(1-(1-0)))}{...}}}}
  \end{itemize}
\end{frame}
