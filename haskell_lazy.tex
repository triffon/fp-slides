...
\date[21.12.17--04.01.18]{21 декември 2017 г. -- 4 януари 2018 г.}
...
\begin{frame}
  ...
  Можем да използваме още функциите:
  \begin{itemize}
  ...
  \item \lst{sorted l = all (\\(x,y) -> (<=) x y) ((zip l . tail) l)}
  \item \lst{sorted l = all (uncurry (<=)) ((. tail) (zip l) l)}
  \item \lst{sorted l = all (uncurry (<=)) ((. tail) . zip) l l)}
  \item \lst{sorted l = all (uncurry (<=)) (join ((. tail) . zip) l)}
  \item \lst{sorted = all (uncurry (<=)) . join ((. tail) . zip)}
  \end{itemize}
\end{frame}
