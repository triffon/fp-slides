...
\begin{frame}[fragile]
  ...
  Можем да дефинираме типове, позоавайки се на самите тях \alert{рекурсивно}.\\[1em]
  \pause
  \textbf{Пример:}
...
\begin{lstlisting}
data BinTree a = Empty | Node { root :: a, left :: BinTree a,
                                           right :: BinTree a }
                 deriving (Eq, Ord, Show, Read)
\end{lstlisting}
  \textbf{Примери:}
...
\begin{frame}[fragile]
  \frametitle{S-списъци}
\small
\begin{lstlisting}
data SList = SBool Bool | SChar Char | SInt Int | SDouble Double |
             SList { getList :: [SList] }
             deriving (Eq, Ord, Show, Read)
\end{lstlisting}
  \ \pause
\begin{lstlisting}
slist = SList [SInt 2, SChar 'a',
                       SList [SBool True, SDouble 1.2, SList []]]
\end{lstlisting}
  \pause
\begin{lstlisting}
countAtoms :: SList -> Int@\pause@
countAtoms (SList sls) = sum $ map countAtoms sls
countAtoms _           = 1
\end{lstlisting}
  \pause
\begin{lstlisting}
flatten :: SList -> SList@\pause@
