...
\begin{frame}[fragile]
  ...
  \item Оценката на булевите константи, знаците, числата и низовете са самите те
    ...
    \item \evalsto 5 5
    \item \evalsto{\#t}{\#t}
    \item \evalsto{\#\textbackslash a}{\#\textbackslash a}
...
\begin{frame}
  ...
  \item \tt{(define (abs x) (if (< x 0) (- x) x))}
  \item \evalsto{(abs -5)}5, \evalsto{(abs (+ 1 2))}3
  \item \tt{(define (f x) (if (< x 5) (+ x 2) "Error"))}
  \item \evalsto{(f 3)}5, \evalsto{(f 5)}{"Error"}
  \item \tt{(define (g x y) (if (< x y) (+ x y) (* x y)))}
  \item \tt{(define (g x y) ((if (< x y) + *) x y))}
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
  \item \tta{(case} <тест> \{\tta({}\{<случай>\} <израз>\tta)\} [\tta{(else} <израз>\tta)]\tta)
...
\begin{frame}[fragile]
  \frametitle{Пример с формата cond}
