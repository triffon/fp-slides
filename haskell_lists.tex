...
\begin{frame}
  ...
  \item Тип кортеж от $n$ елемента: \tt($t_1$\tt, $t_2$\tt, $\ldots$\tt, $t_n$\tt)
  \item Стойности: наредени $n$-торки от вида \tt($x_1$\tt, $x_2$\tt, $\ldots$\tt, $x_n$\tt), където $x_i$ е от тип $t_i$
...
\begin{frame}[fragile]
  ...
  Образец на кортеж е конструкция от вида \tt($p_1$\tt, $p_2$\tt, $\ldots$\tt, $p_n$\tt).\\\pause
  Пасва на всеки кортеж от точно $n$ елемента \tt($x_1$\tt, $x_2$\tt, $\ldots$\tt, $x_n$\tt), за който образецът $p_i$ пасва на елемента $x_i$.
  \onslide<+->
  \begin{itemize}[<+->]
...
\begin{frame}
  ...
  \item \tt[$a_1$\tt, $a_2$\tt, $\ldots$\tt, $a_n$\tt] e по-удобен запис за $a_1$\tt{:(}$a_2$\tt:\ldots($a_n$\tt{:[])}\ldots\tt)
...
\begin{frame}<1-14>[fragile]
  ...
  \item \lst{foldr op nv} \tt[$x_1$\tt, $x_2$\tt, \ldots\tt, $x_n$\tt] = \\
...
\begin{frame}[fragile]
  ...
  \item \lst{foldl op nv }\tt[$x_1$\tt, $x_2$\tt, \ldots\tt, $x_n$\tt] = \\
...
\begin{frame}[fragile]
  ...
  \item \lst{foldr1 op nv }\tt[$x_1$\tt, $x_2$\tt, \ldots\tt, $x_n$\tt] = \\
...
\end{lstlisting}
  ...
  \item \lst{foldl1 op nv }\tt[$x_1$\tt, $x_2$\tt, \ldots\tt, $x_n$\tt] = \\
...
\begin{frame}
  ...
  \item \lst{scanr op nv} \tt[$x_1$\tt, $x_2$\tt, \ldots\tt, $x_n$\tt] = \\
  ...
  \item \lst{scanl op nv} \tt[$x_1$\tt, $x_2$\tt, \ldots\tt, $x_n$\tt] = \\
    \tt{[nv,}\\
    \tt{ nv `op` }$x_1$\tt,\\
...
\begin{frame}
  ...
  \item \lst{zip :: [a] -> [b] -> [(a,b)]}
    \begin{itemize}
    \item \evalstos{\lst{zip [}$x_1$\tt, $x_2$\tt, \ldots\tt, $x_n$\tt]
        \tt[$y_1$\tt, $y_2$\tt, \ldots\tt, $y_n$\tt]}{\tt[($x_1$\tt, $y_1$\tt{), (}$x_2$\tt, $y_2$\tt{), }\ldots\tt{, (}$x_n$\tt, $y_n$\tt{)]}}
  ...
  \item \lst{unzip :: [(a,b)] -> ([a],[b])}
    ...
    \item \evalstos{\lst{unzip}
        \tt{[(}$x_1$\tt,$y_1$\tt{), (}$x_2$\tt,$y_2$\tt{), }\ldots\tt{, (}$x_n$,$y_n$\tt{)]}}{\tt{([}$x_1$\tt,$x_2$\tt, \ldots\tt,$x_n$\tt{], [}$y_1$\tt,$y_2$\tt, \ldots\tt, $y_n$\tt{])}}
    \end{itemize}
  \item \lst{zipWith :: (a -> b -> c) -> [a] -> [b] -> [c]}
    \begin{itemize}
    \item съшива два списъка с дадена бинарна операция
    \item \evalstos{\lst{zipWith op} $x_1$\tt,$x_2$\tt,\ldots\tt,$x_n$\tt]
        \tt[$y_1$\tt, $y_2$\tt, \ldots\tt,$y_n$\tt]}{\tt[$x_1$\tt{ `op` }$y_1$\tt, $x_2$\tt{ `op` }
        $y_2$\tt, \ldots\tt, $x_n$\tt{ `op` }$y_n$]}
    \end{itemize}
  \item Примери
    \begin{itemize}
    ...
    \item \lst{unzip = foldr (\\(x,y) (l1,l2) -> (x:l1,y:l2)) ([],[])}
    \end{itemize}
  \end{itemize}
\end{frame}
