...
\alt<2>{((1 2 3))
 ...
 \end{semiverbatim}
  ...
  \end{columns}
  \vspace{1em}
  Асоциативен списък, в който \alert{ключовете} са върховете, а \alert{стойностите} са списъци от техните наследници.
\end{frame}
...
\end{verbatim}
  ...
  \item \alert{Какво се случва ако графът е цикличен?}
    \begin{itemize}
    \item Програмата също зацикля!
    \item Как да се справим с този проблем?
...
\begin{frame}[t,fragile]
  ...
  \begin{itemize}
  \item \tt u = \tt v, или
  \item има път от някое от децата на \tt u до \tt v
  \end{itemize}
  \onslide<+->
    \small
...
\alt<6->{(define (dfs-path u v g)
  (define (dfs-path-search path)
    (cond ((eq? (car path) to) (reverse path))
          ((memq (car path) (cdr path)) #f)
          (else (g 'search-child (car path)
                  (lambda (c) (dfs-path-search (cons c path)))))))
  (dfs-path-search (list from)))}{(define (dfs-path u v g)
  (if (eq? u v) (list u)
      (g 'search-child u (lambda (c)
                           (cons#f u (dfs-path c v g))))))}
\end{semiverbatim}
    \end{overlayarea}
    \onslide<+->
    \alert{Рекурсивно решение, работи само за ацикличен граф!}\\
    \onslide<+->
    Итеративното решение ни позволява да запазим пътя намерен до момента.
\end{frame}
