...
(define (make-graph g)
  (define (self prop . params)
    (case prop
      ('print g)
      ('vertices (map car g))
      ('successors (let ((v (car params)))
                     (cdr (assq v g))))             ;; \alert{\([s|s\leftarrow{}n]\)}
      ('edge? (let ((u (car params)) (v (cadr params)))
                (memq v (self 'successors u))))     ;; \alert{\(u\stackrel?\rightarrow{}v\)}
      ('map-successors (let ((v (car params))
                             (f (cadr params)))     ;; \alert{\(\forall{}s\leftarrow{}n\)}
                         (map f (self 'successors v))))
      ('exists-successor (let ((v (car params))
                               (f (cadr params)))  ;; \alert{\(\exists{}s\leftarrow{}n\)}
                           (exists f (self 'successors v))))))
  self)
\end{semiverbatim}
\end{frame}
...
\begin{verbatim}
(define (childless g)
  (filter (lambda (v) (null? (g 'successors v))) (g 'vertices)))
\end{verbatim}
  \pause
  \vspace{1em}
  \textbf{Задача. }Да се намерят предшествениците на даден връх.\\
  \pause
  \textbf{Решение. }\tt{predecessors(v,g)} $ = \{u | u \leftarrow v \}$
  \pause
\begin{verbatim}
...
\begin{semiverbatim}
(define (forall f l)
  (not (exists (lambda (x) (not (f x))) l)))\pause
