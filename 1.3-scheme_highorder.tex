...
\begin{frame}[fragile]
  ...
  \begin{overlayarea}{\textwidth}{.87\textheight}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{.5\textwidth}
      \begin{tikzpicture}
        ...
        \childenvir[2- ]{below left =2em and -5em}E{E_1}{\kv n1}
        \childenvir[4- ]{below right=2em and -5em}E{E_2}{\kv n5}
        \childenvir[7- ]{below=2em}{E_1}{E_3}{\kv i7}
        \childenvir[10-]{below=2em}{E_2}{E_4}{\kv i7}
      \end{tikzpicture}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}
  ...
  Специалната форма \tt{lambda} е достатъчна за реализацията на (почти) всички конструкции в Scheme!\\[1em]
  \pause
  Симулация на \tt{let}:
  \begin{tabular}{c}
...
\begin{frame}[fragile]
  ...
  \textbf{Идея:} представяне на числото $n$ като $\lambda{}f,x \;\;f^n(x)$\\[1em]
  \pause
  \begin{itemize}[<+->]
...
\begin{frame}[fragile]
  ...
  \pause
  \textbf{Проблем №2:} \lst{gamma-inf} отново използва рекурсия\ldots\\[1em]
  \pause
