      ...
      \begin{column}{.5\textwidth}
        \begin{center}
          \evalchain{
            ...
            "\lst{(define 1+ (n+ 1))}"[envp=E,skip] -!-
            "\lst{(define 5+ (n+ 5))}"[envp=E,skip] -!-
            "\lst{(1+ 7)}"[env=E] ->
...
\begin{frame}[fragile]
  ...
  \begin{fixedarea}
  Да се намери $n$-та производна на дадена едноместна функция.\\[2ex]
  \pause
...
\begin{frame}
  ...
  Специалната форма \tt{lambda} е достатъчна за реализацията на (почти) всички конструкции в Scheme!\\[2ex]
  \pause
  Симулация на \tt{let}:
  \begin{tabular}{c}
